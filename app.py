...
CONTRACT_RATE_CARDS = {
    "HL-ASIA-2026": {
        "base_rate": 3122.00,  # Agreed contract rate benchmark ($3,122.00)
        "contracted_baf": 0.00,
        "contracted_thc": 0.00,
        "contracted_pss": 0.00,
    }
...
def parse_invoice_data(text):
    ...
    # Total Billed extraction (matches TOTAL AMOUNT DUE (USD) $6,513.45)
...
if uploaded_file is not None:
    ...
    if overcharge_leakage > 0:
        ...
        st.markdown(f"""
        * **Weight Extracted:** Invoice weight is **{data['gross_weight']:,.2f} KG** ({actual_unit_weight:.1f} kg/ctn).
